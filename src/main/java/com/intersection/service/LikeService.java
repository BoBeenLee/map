...
public class LikeService {
	...
	public void addLike(String phoneId, int transNo) {
		Query query = em.createQuery("insert into Like(userNo, transNo)"
				+ " select userNo, :transNo"
				+ " from User u"
	...
	public List<Like> getLikesByUserNo(int userNo) {
		CriteriaQuery<Like> c = em.getCriteriaBuilder().createQuery(
				Like.class);
		c.from(Like.class);
	...
	public Like getLikesByPhoneId(String phoneId, int transNo) {
		TypedQuery<Like> query = em.createQuery("select L"
				+ " from Like L, User U"
	...
	public List<Translation> getLikesByNames(List<String> names, String phoneId) {
		 Query query = em.createNativeQuery("select trans.trans_no, trans.name, trans.type, "
		 	+ " (select count(*)"
		 	+ " from is_trans_like_db"
			+ " where trans_no = trans.trans_no and user_no = usr.user_no) as like_count, (usr.phone_id = :phoneId and phone_id is not null) as like_status"
		 	...
		 	+ " group by trans.trans_no, trans.name, trans.type, usr.user_no, usr.phone_id"
		 	+ " order by like_count;");
